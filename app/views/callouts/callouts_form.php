...
<form action='<?= app("url") ?>/exec/callouts/action/' id='calloutForm' name='calloutForm' method="post">
    <div style="float:left;width:640px;background-color:#f9f9f9;padding:10px;border:1px solid #ccc;">
        ...
        <?parentListReq("frm_callout_status_id","_callout_status",$values["callout_status_id"],"callout_status_name","")?><br>
        ...
        <select id="frm_job_id" name="frm_job_id" class="required">
            ...
            <option <?if ($row["job_id"]==$values["job_id"]){?>
                SELECTED
                <?}?>
        ...
        <label>Fault / Priority <b>(COMPLAINT)</b></label>
        <?parentListReq("frm_fault_id","_faults",$values["fault_id"],"fault_name","")?>
        ...
        <div style="display: none;">
            <label>Attributable</label>
            <?parentListReq("frm_attributable_id","_attributable",$values["attributable_id"],"attributable_name","")?>
        </div>
        <br>
        <?
                        $checked = "";
    ...
    <div style="margin-left:5px;float:left;width:550px;background-color:#faffb2;padding:10px;border:1px solid #ccc;">
        ...
        <label>Fault Found <b>(CAUSE)</b></label>
        <?parentList("frm_technician_fault_id","_technician_faults",$values["technician_fault_id"],"technician_fault_name","where tech_hidden is null order by technician_fault_name ASC")?><br>

        <label>Work Done <b>(CORRECTION)</b></label>
        <?parentList("frm_correction_id","_corrections",$values["correction_id"],"correction_name","")?><br>
...
<script>
    $(document).ready(function() {
        ...
        if (isMobile() === false) {
            $('#frm_callout_time').datetimepicker({
                dateFormat: 'dd-mm-yy',
                timeFormat: 'HH:mm:ss',
                showSecond: false
            });
            $('#frm_time_of_arrival').datetimepicker({
                dateFormat: 'dd-mm-yy',
                timeFormat: 'HH:mm:ss',
                showSecond: false
            });
            $('#frm_rectification_time').datetimepicker({
                dateFormat: 'dd-mm-yy',
                timeFormat: 'HH:mm:ss',
                showSecond: false
            });
            $('#frm_time_of_departure').datetimepicker({
                dateFormat: 'dd-mm-yy',
                timeFormat: 'HH:mm:ss',
                showSecond: false
            });
